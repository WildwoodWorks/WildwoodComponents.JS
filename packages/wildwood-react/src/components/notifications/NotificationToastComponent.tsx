...
export function NotificationToastComponent({
  ...
  const getTypeClass = (type: string) => {
    switch (type) {
      case 'success': return 'ww-toast-success';
      case 'error': return 'ww-toast-error';
      case 'warning': return 'ww-toast-warning';
      case 'info': return 'ww-toast-info';
      default: return 'ww-toast-info';
    }
  };
  ...
  return (
    <div className={`ww-toast-container ww-toast-${position} ${className ?? ''}`}>
      {visibleToasts.map((toast) => (
        <div
          key={toast.id}
          className={`ww-toast ${getTypeClass(toast.type)}`}
          role="alert"
        >
