...
export function usePayment(): UsePaymentReturn {
  ...
  const confirmPayment = useCallback(
    async (paymentIntentId: string, providerType: PaymentProviderType, confirmationData?: Record<string, unknown>) => {
      setLoading(true);
      setError(null);
      try {
  ...
  const validateAppStoreReceipt = useCallback(
    async (receiptData: string, providerType: PaymentProviderType) => {
