...
export interface RegistrationSubscriptionLabels {
  ...
  /**
   * Said when a purchase needs a card challenge the host cannot answer — a stack with no
   * `PaymentActionAdapter` carries no payment SDK, so a 3-D Secure prompt has nowhere to run.
   * Never a silent failure: the customer is told where to finish it instead.
   */
  finishOnWeb: string;
