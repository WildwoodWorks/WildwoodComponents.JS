...
//
// It is DOM-free on purpose, so React Native shares it rather than copying it: the two web words
// the original carried — `platform` and `deviceInfo` — are options the host fills in, the same way
// `useAuthenticationLogic` takes them.
...
import { useRegistrationSubscription } from '../hooks/useRegistrationSubscription.js';
import {
  initialSignupState,
...
/**
 * Everything {@link useSignupFlow} needs that carries no UI. A view's own props (a class name, a
 * `renderClosed` node) are its package's business and are simply passed straight through.
 */
export interface SignupFlowOptions {
  ...
  currency?: string;
  /** Overrides for any of the flow's strings. Unset keys keep the shipped copy. */
  labels?: Partial<RegistrationSubscriptionLabels>;
  /** Called whenever the flow gives up on something. Never throws into the host's render. */
  ...
  /**
   * When the plan's card is taken. Default `'beforeAccount'` — the web order. A store-billed stack
   * passes `'afterAccount'` so a purchase is never made before there is an account to attach it to.
   */
  paymentOrder?: SignupPaymentOrder;
  /** What the registration is told it was made from. `'web'`, or React Native's `Platform.OS`. */
  platform?: string;
  /** The device the registration is told about: a user agent, or `${Platform.OS} ${Platform.Version}`. */
  deviceInfo?: string;
...
export interface SignupFlow {
  ...
  /**
   * Whether the card on screen is the ACCOUNT-FIRST one: the account already exists, so backing
   * out of it finishes the signup through {@link SignupFlow.paymentAbandoned} rather than stepping
   * back. Always false in the pay-first order.
   */
...
export function useSignupFlow(props: SignupFlowOptions): SignupFlow {
  const {
    appId,
    currency,
  ...
  // The machine's own answer to "is there still a card to take", for the callbacks that run
  // outside the driver effect and must not re-derive it.
  const machine = useRef<SignupState>(state);
  ...
  /**
   * Attach the plan's payment to the account and start the subscription — the tail of the wizard's
   * `processSignup`. Pay-first runs it inside `creating`, where the card was taken minutes ago;
   * account-first runs it the moment the card comes back, because until then there is nothing to
   * subscribe with. Neither is fatal: the account exists, and a plan can be started from the
   * dashboard.
   */
  const activatePlan = useCallback(async () => {
    ...
    // The plan's card may have been taken before the account existed, so the transaction belongs
    // to nobody until now. The server looks it up by the provider's own id.
  ...
  const runSignup = useCallback(
    async (token: StepToken) => {
      ...
      try {
        ...
        // Account-first with a card still to take: the plan is activated by `paymentSucceeded`,
        // once there is a transaction to subscribe with. The machine decides, so it is asked.
        const current = machine.current;
        const deferPlan =
          current.options.paymentOrder === 'afterAccount' &&
          !current.paymentTransactionId &&
          signupPlanNeedsPayment(current);
        if (!deferPlan) await activatePlan();
    ...
    [client, resolvedAppId, platform, deviceInfo, activatePlan],
  );
  ...
  // unless the account already exists: then there is no form to go back to, and the signup
  // finishes with the plan's activation pending instead of stranding a real account.
  useEffect(() => {
    ...
    dispatch({ type: 'GO_TO', step: 'register' });
  }, [state.step, state.formSubmitted, state.paymentAfterAccount, plan]);
  ...
  const paymentSucceeded = useCallback(
    (result: PaymentCompletionResult) => {
      ...
      const paymentTransactionId = result.transactionId ?? result.paymentIntentId ?? '';
      // Account-first: the account is already there, so this is the moment the transaction can be
      // attached to it and the plan started — the same work `creating` does in the pay-first order,
      // which is why it is the same function.
      if (machine.current.paymentAfterAccount) {
        void (async () => {
          await activatePlan();
  ...
  const paymentAbandoned = useCallback(() => {
    dispatch({ type: 'PAYMENT_ABANDONED' });
  }, []);
  ...
  return {
    ...
    trialDays,
    subscriptionFailed,
    paymentAfterAccount: state.paymentAfterAccount,
    processingStatus,
    formData,
    initialFormData,
    checkoutItems,
